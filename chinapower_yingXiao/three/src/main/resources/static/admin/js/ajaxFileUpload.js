jQuery.extend({
			...
			createUploadIframe : function(id, uri) {// id为当前系统时间字符串，uri是外部传入的json对象的一个参数
				...
				var iframeHtml = '<iframe id="'
						...
						+ '" name="'
						+ frameId
						+ '" style="position:absolute; top:-9999px; left:-9999px"'; // 创建iframe元素
			...
			createUploadForm : function(id, fileElementId, data) {// id为当前系统时间字符串，fileElementId为页面<input
																	// type='file'
																	// />的id，data的值需要根据传入json的键来决定
				// create form
				var formId = 'jUploadForm' + id; // 给form添加一个独一无二的id
				var fileId = 'jUploadFile' + id; // 给<input type='file'
													// />添加一个独一无二的id
				var form = jQuery('<form  action="" method="POST" name="'
						+ formId + '" id="' + formId
						+ '" enctype="multipart/form-data" ></form>'); // 创建form元素
				if (data) {// 通常为false
					for ( var i in data) {
						jQuery(
								'<input type="hidden" name="' + i + '" value="'
										+ data[i] + '" />').appendTo(form); // 根据data的内容，创建隐藏域，这部分我还不知道是什么时候用到。估计是传入json的时候，如果默认传一些参数的话要用到。
					}
				}
			...
			ajaxFileUpload : function(s) {// 这里s是个json对象，传入一些ajax的参数
				...
				var form = jQuery.createUploadForm(id, s.fileElementId,
						(typeof (s.data) == 'undefined' ? false : s.data)); // 创建动态form
					...
					if (xml || isTimeout == "timeout") {// xml变量被赋值或者isTimeout
						...
						try {
							status = isTimeout != "timeout" ? "success"
									: "error"; // 如果不是“超时”，表示请求成功
							// Make sure that the request was successful or
							// notmodified
							if (status != "error") { // process the data
														// (runs the xml through
														// httpData regardless
														// of callback)
								var data = jQuery.uploadHttpData(xml,
										s.dataType); // 根据传送的type类型，返回json对象，此时返回的data就是后台操作后的返回结果
						...
						if (s.global && !--jQuery.active)
							jQuery.event.trigger("ajaxStop"); // Process
																// result
						if (s.complete)
				...
				if (s.timeout > 0) {// 超时检测
					setTimeout(function() { // Check to see if the request is
											// still happening
						if (!requestDone)
				...
				jQuery('#' + frameId).load(uploadCallback); // ajax
															// 请求从服务器加载数据，同时传入回调函数
				return {
					abort : function() {
					}
			...
			uploadHttpData : function(r, type) {
				...
				data = type == "xml" || data ? r.responseXML : r.responseText; // If
																				...
																				// context
				if (type == "script")
					jQuery.globalEval(data); // Get the JavaScript object, if
												// JSON is used.
				if (type == "json")
