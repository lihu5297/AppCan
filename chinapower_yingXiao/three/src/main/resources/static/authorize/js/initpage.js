...
$(function() {
	$("#authorizepath").change(function() {
		$.ajaxFileUpload({
			url : basePath+"setting/upload", //用于文件上传的服务器端请求地址
			secureuri : false, //一般设置为false
			fileElementId : "authorizepath" , //文件上传控件的id属性  <input type="file" id="file" name="file" /> 注意，这里一定要有name值   
			...
			success : function(data, status) //服务器成功响应处理函数
			{
	            ...
	            if(data.substring(0, 1) == 0){     //0表示上传成功(后跟上传后的文件路径),1表示失败(后跟失败描述)
	                $("#authorizePath").val(data.substring(2));
	                alert("授权文件上传成功");
	            }else{
	            	alert('授权文件上传失败，请重试！！');
	            }
...
$("#saveButton").click(function() {
	...
	ajax_url += "setting/updateAuthorize?type=platForm&info="+name;
